" Syntax file for Gavin's custom Todo language
...
" syntax keyword todoNone None
syntax keyword todoClassCS CS 
syntax keyword todoClassMath MATH Math

""""""""""""""""""""""""""""""""""""""""""""""""""
" Matches
""""""""""""""""""""""""""""""""""""""""""""""""""
syntax match todoNone /- None/
syntax match todoDivider /^\s*##*$/
" The match encodes the following
"	\^ means look for line start
"	\s* means scan through any white space
"	>>> means match the literal characters of >>>
"	.* means scan through any remaining characters
"	$ means stop scanning at line end
syntax match todoCompleteStatus /\[done\].*$\|\[Done\].*$/
syntax match todoCurTask /^\s*>>>>\s.*$/
syntax match todoHighPriority /^\s*>>>\s.*$/
syntax match todoMedPriority /^\s*>>\s.*$/
syntax match todoLowPriority /^\s*>\s.*$/
...
""""""""""""""""""""""""""""""""""""""""""""""""""
" highlight {def_name} ctermbg=xx ctermfg=xx guifg=#xxxxxx guibg=#xxxxxx
...
hi todoCurTaskHighlight	ctermfg=196 ctermbg=231
""""""""""""""""""""""""""""""""""""""""""""""""""
" Highlights
""""""""""""""""""""""""""""""""""""""""""""""""""

" keyword highlights
highlight def link todoNone x239_Grey30
highlight def link todoClassCS x129_Purple
highlight def link todoClassMath x033_DodgerBlue1

" match lightlights
highlight def link todoDivider x045_Turquoise2
highlight def link todoCompleteStatus x071_DarkSeaGreen4 
highlight def link todoCurTask todoCurTaskHighlight
highlight def link todoHighPriority x196_Red1
highlight def link todoMedPriority x172_Orange3 
highlight def link todoLowPriority x184_Yellow3
...
" handle main_syntax variable stuff
if main_syntax == 'todo'
	unlet main_syntax
endif

" Return to cpo option to what it was
let &cpo = s:cpo_save
unlet s:cpo_save
